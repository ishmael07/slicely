...
import { centsToMicros, formatMoney } from "../../main/pricing";
import { EmailRejected, isDisposableDomain, normalizeEmail } from "../../main/accounts/email";
import { accountExistsFor, balanceMicros, findOrCreateAccount, getAccount, type Account } from "../../main/accounts/store";
...
export function createAuthRouter(cfg: OauthConfig = {}): Router {
  ...
  router.get("/:provider/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const first = !accountExistsFor(provider.id, raw.providerUserId, email.normalized);
      if (first) {
...
function meFor(req: Request, cfg: OauthConfig): MeResponse {
  ...
  return { signedIn: true, account: toAccountView(account) };
}

/** An account as the client is allowed to see it: an email, a monogram, four
 *  integers and two strings formatted ONCE, here, so two clients cannot
 *  disagree about what 45,286,000 µ¢ says. */
function toAccountView(account: Account): AccountView {
  ...
  const view: AccountView = {
    email: account.email,
    initial: initialOf(account.email),
  ...
  if (account.name) view.name = account.name;
  return view;
}

/** One uppercase letter for the monogram, or `"?"` for an address that starts
 *  with something that is not a letter (and so would draw as a glyph nobody
