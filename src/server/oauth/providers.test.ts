...
test("a Google sign-in writes nothing to disk and makes exactly one outbound call", async () => {
  ...
  allSet();
  try {
    // Resolve the config FIRST: getConfig() scaffolds `downloads/` and `slices/`
    // under a fresh workdir on its first call, and that is the server booting,
    // not the sign-in writing. The snapshot is taken after it so this test keeps
    // saying what it means — the sign-in itself leaves nothing behind.
    getConfig();
