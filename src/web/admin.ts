...
function usd(micros: number | null): string {
  ...
  return dollars > 0 && dollars < 0.01 ? `$${dollars.toFixed(3)}` : `$${dollars.toFixed(2)}`;
}
...
function bars(s: AdminSummary): HTMLElement {
  ...
  for (const d of s.usage.days) {
    ...
    b.append(el("b", "", `${usd(d.spendMicros)} · ${d.calls} calls`), i, el("span", "", d.day.slice(5)));
    b.title = `${d.day}: ${usd(d.spendMicros)} spent, ${d.calls} calls, ${d.tokensIn.toLocaleString()} in / ${d.tokensOut.toLocaleString()} out`;
    g.append(b);
  }
  wrap.append(g);
  return wrap;
...
function render(s: AdminSummary): void {
  ...
  root.append(
    tiles([
      ...
      tile("Calls today", String(s.usage.callsToday), `${s.usage.calls7d} this week`),
  ...
  root.append(
    s.usage.byModel.length
      ? table(
          ["Model", "Calls", "Cost"],
          s.usage.byModel.map((m) => [m.model, String(m.calls), usd(m.micros)]),
          [1, 2],
        )
      : el("div", "empty-note", "No metered calls in the last 14 days."),
  );
  ...
  root.append(
    s.accounts.length
      ...
      : el("div", "empty-note", "Nobody has signed in yet."),
  );
...
function gate(): void {
  ...
  const p = el("p", "", "Nothing here for this account. ");
  const a = el("a", "", "Sign in to Slicely");
  a.href = "/";
  p.append(a, document.createTextNode(" as the owner, then come back."));
  g.append(p);
  root.append(g);
}
