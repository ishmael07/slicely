...
- Wire errors are `{ error: string, code?: string }`. Stable codes: `no_key`, `key_rejected`, `key_invalid_format`, `rate_limited`, `slicer_busy`, `not_in_workspace`, `forbidden_in_hosted_mode`, `cross_origin`, `billing`, `busy`, `not_found`, `too_large`, `zip_too_many_entries`, `slice_timeout`, `host_blocked`. (Spec §Error handling.)
