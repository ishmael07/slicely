...
- Wire errors stay `{ error: string, code?: string }`. Existing stable codes are unchanged. **New stable codes, and there are exactly eight:** `signin_required`, `credit_exhausted`, `free_tier_paused`, `signup_limited`, `email_unverified`, `email_blocked`, `account_blocked`, `oauth_failed`. Each gets a sentence in `CODE_COPY` (`src/web/api.ts`). `email_invalid` is a waitlist-only 400 and is not a chat code. (Spec §10.5.)
- **`account_blocked` is 409, "This account can't use Slicely.", and it is refused FIRST** — before the own-key branch and before any provider call (`main/agent/funding.ts`'s `refuseIfBlocked`). A blocked account holding a key of its own is still refused; blocked means blocked. Signing in stays allowed (`routes/auth.ts` binds a blocked account on purpose) so the person reads the sentence instead of hitting a silent dead end, and the record — including its sign-in history — stays there for the owner. `email_blocked` keeps its own, narrower job: that ADDRESS (a disposable domain) cannot be used, reported in the callback's `#auth_error=` fragment.
- **No raw IP is ever persisted or sent.** An account's `signins` history keeps `signups.ts`'s `hashIp` — `sha256(salt ‖ ip)` truncated to 128 bits under this machine's own salt — and nothing else. Same hash as the per-address signup cap, so two accounts from one address are comparable without either file holding an address. (Spec §8.)
